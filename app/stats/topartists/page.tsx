        ...
        <div className="h-full flex-1 overflow-y-auto border-t border-gray-100 border-b rounded bg-gray-100">
          ...
          <div className="py-10">
            ...
            <div className="mt-12 grid grid-cols-1 gap-8 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              {currentView === 'artists' && filteredArtists.map((artist, index) => (
                <div key={artist.id} className="flex items-center space-x-4">
                  <span className="text-lg font-medium">{index + 1}.</span>
                  <img src={artist.artist_url} alt={artist.artist} width="50" height="50" />
                  <div>
                    <div className="font-medium text-lg">{artist.artist}</div>
                  </div>
                </div>
              ))}
            </div>
